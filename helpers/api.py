...
class VeracodeAPI:
    ...
    def _rest_paged_request(self, url, method, element, params=None):
        if params==None:
            params = {'page': 0} #initialize the page request
    ...
    def update_sandbox(self,sandbox_id,operation,value1,value2=None):
        if operation == "autorecreate":
            call_params = {"sandbox_id": sandbox_id,"auto_recreate": value1}
        elif operation == "customfield":
            call_params = {"sandbox_id": sandbox_id,"custom_field_name": value1,"custom_field_value":value2}
        else:   
            return

        response = self._request(self.baseurl+"/5.0/updatesandbox.do", "GET", call_params)
        return response
    ...
    def get_app_sandboxes(self,guid):
        """Fetches a list of sandboxes for the application using the Veracode Appplication API."""
        apps_base_uri = "appsec/v1/applications" + "/{}/sandboxes"
        uri = apps_base_uri.format(guid)      
